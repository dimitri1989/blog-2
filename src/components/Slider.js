   ...
   return  <div key={index} className={index === 0 ? "carousel-item active" : "carousel-item" } data-bs-interval="10000">
    ...
    <div className="news-content-title text-slider-title">
      ...
      <div className='d-flex justify-content-center mt-5 control-bluebtn'>
                    <button type='button' className='readon-btn me-3 typology-pagination bg-light text-info typology-pagination2 bluebtn'><Link to={`/${data.id}`}>READ ON</Link></button>
                    <button type='button' className=' typology-pagination readon-btn border'><HiOutlineBookmark className='icon' /><Link to={`Editingnews/${data.id}`}>EDIT BLOG</Link></button>
