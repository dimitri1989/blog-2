...
//const userinf= userInfo(users,key)
  var createdTime =  blog && blog.created_at.slice(0, -17)
    ...
    //text border and 
    const [text,setText] = useState({title:"divlabel",email:"divlabel",name:"divlabel"})
    function onChange(e,name){
      ...
      function checkInputs(names,sendValue){
        ...
        if(sendValue[names] !== undefined){
          if(!sendValue[names].length > 0){
          
            setText({...text,[names]:'text-danger divlabel'})
            //console.log(sendValue[names].length, [names]);
         }
         
         else{
          setText({...text,[names]:'divlabel'})
         }
  ...
  return (
    <div className='comments'>
        ...
        <form onSubmit={(e) => addNewsHandler(e)}>
              ...
              <textarea id="comment" value={sendValue.title}  rows="10"  cols="80" onChange={(e)=> onChange(e,"title")}/>
              <div className='input-forms  d-flex justify-content-between'>
              <label for="email" className={text.email}>Email *<input type='email' value={sendValue.email} onChange={(e)=> onChange(e,"email")} /> </label>
              <label for="username" className={text.name}>Name *<input type='text' value={sendValue.name} onChange={(e)=> onChange(e,"name")} /></label>
              <label for="username" className={text.website}>Website <input type='text' value={sendValue.website} onChange={(e)=> onChange(e,"website")} /></label>
